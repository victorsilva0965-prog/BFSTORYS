...
async function carregarDados() {
    try {
        // 1. Limpeza de travamento inicial
        document.body.style.overflow = 'auto';
        
        const response = await fetch('produtos.json?v=' + Date.now());
        const data = await response.json();

        // 2. Compatibilidade de formato (Lista ou Objeto)
        ...
        if(v) v.innerHTML = "<p>Erro ao carregar produtos. Verifique sua conexão.</p>";
    }
}
...
// FUNÇÕES DE INTERAÇÃO (SIDEBAR / MODAL)
function toggleMenu() {
